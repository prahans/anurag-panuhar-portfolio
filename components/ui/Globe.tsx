...
export function Globe({ globeConfig, data }: WorldProps) {
  ...
  useEffect(() => {
    ...
    // Point geometry needs endpoint coordinates, not the arc records.
    globe
    ...
    globe
      ...
      .ringLat("lat")
      .ringLng("lng")
      .ringColor((e: object) => (e as GlobePoint).ringColor)
  ...
  useEffect(() => {
    const globe = globeRef.current;
    if (!globe || !globeData.length || defaultProps.rings <= 0) return;
    ...
    const interval = setInterval(() => {
      ...
      globe.ringsData(globeData.filter((_, i) => numbersOfRings.includes(i)));
    }, 2000);

    return () => {
      clearInterval(interval);
    };
  }, [globeData, defaultProps.rings]);
